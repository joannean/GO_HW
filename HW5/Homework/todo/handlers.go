...
func deleteTodo(c *gin.Context) {
	...
	for i, todo := range globalTodo {
		if todo.ID == uint(id) { // 將 id 轉換為 uint
			...
			message := fmt.Sprintf("ID %d Todo deleted", deletedTodo.ID)
...
func addTodoToQueue(c *gin.Context) {
	var todo TODO
	...
	err = redisClient.LPush(context.Background(), "queue", string(b)).Err()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "Todo added to queue"})
}
